	...
	var WordFindGame = function() {
		...
		let 
			puzzleId  = 'puzzle'
				// Content button
				, puzzleRow  = 'puzzleRow'
					// Content puzzleSquare
					, puzzleSquare  = 'puzzleSquare'
						// States puzzleSquare
						, puzzleSelected  = 'puzzleSelected'
						, puzzleFound 		= 'puzzleFound'
						, puzzleSolved  	= 'puzzleSolved'
						, puzzleComplete  = 'puzzleComplete'
				// Word finded
				, puzzleWords 		= 'puzzleWords'
				, puzzleWord 		= 'puzzleWord'
				, puzzleWordFound = 'puzzleWordFound'
		;
			...
			for (var i = 0, height = puzzle.length; i < height; i++) {
				...
				output += '<div class="'+puzzleRow+'">';
					...
					for (var j = 0, width = row.length; j < width; j++) {
							...
							output += '<span class="'+puzzleSquare+'_word">';
							output += row[j] || '&nbsp;';
							output += '</span>';
							output += '</button>';
					}
		...
		*/
		var drawWords = function (el, words) {
			...
			var output = '<ul class="'+puzzleWords+'">';
			for (var i = 0, len = words.length; i < len; i++) {
				var word = words[i];
				output += '<li class="puzzleWord ' + word + '">' + word;
			}
			output += '</ul>';
		...
		*/
		var select = function (target) {
			...
			while (backTo < selectedSquares.length) {
				$(selectedSquares[selectedSquares.length-1]).removeClass(`${puzzleSelected}`);
			...
			if (newOrientation) {
				...
				if (lastSquare !== startSquare) {
					$(lastSquare).removeClass(`${puzzleSelected}`);
		...
		var playTurn = function (square) {
			...
			for (var i = 0, len = wordList.length; i < len; i++) {
				if (wordList[i].indexOf(curWord + $(square).text()) === 0) {
					$(square).addClass(`${puzzleSelected}`);
		...
		var endTurn = function () {
			...
			for (var i = 0, len = wordList.length; i < len; i++) {
				...
				if (wordList[i] === curWord) {
					$(`.${puzzleSelected}`).addClass(`${puzzleFound}`);
					wordList.splice(i,1);
					$('.' + curWord).addClass(`${puzzleWordFound}`);
				}

				if (wordList.length === 0) {
					$(`.${puzzleSquare}`).addClass(`${puzzleComplete}`);
					$('#' + puzzleId).addClass(`${puzzleComplete}`);
				}
			}

			// reset the turn
			$(`.${puzzleSelected}`).removeClass(`${puzzleSelected}`);
			startSquare = null;
			selectedSquares = [];
			curWord = '';
			curOrientation = null;
			...
			*/
			solve: function(puzzle, words) {
				...
				for( var i = 0, len = solution.length; i < len; i++) {
					...
					if (!$('.' + word).hasClass(`${puzzleWordFound}`)) {
						for (var j = 0, size = word.length; j < size; j++) {
							var nextPos = next(x, y, j);
							$('[x="' + nextPos.x + '"][y="' + nextPos.y + '"]').addClass(`${puzzleSolved}`);
						}

						$('.' + word).addClass(`${puzzleWordFound}`);
					}
				}
