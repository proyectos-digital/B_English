...
// Controllador global
app.controller('CtrlActivities', ['$scope', '$state', '$stateParams', 'ngAudio', '$timeout', '$http','funcionesGeneral','Upload', 'ApiActividades', 'ApiUnidades',
            function($scope, $state, $stateParams, ngAudio, $timeout, $http, funcionesGeneral,Upload, ApiActividades, ApiUnidades){
    /*
     *
    ...
    */
    let
        ...
        , closeNotice = [...document.querySelectorAll('.closeNotice')]
        , open_Notice = [...document.querySelectorAll('.openNotice')]
    ...
    */
    ...
    $scope.triggerNoticesAside = (accion, options = undefined) => {triggerNoticesAside(accion, options = undefined)};
    ...
    if(estadoRuta.name == 'test'){
        $scope.categoriaid = 3;
        if (document.querySelectorAll('.u-bgIconRefresh')) document.querySelectorAll('.u-bgIconRefresh').forEach((elm, i) => elm.classList.add('is-hidden') )
    }
    if(estadoRuta.name == 'grammar'){   $scope.categoriaid = 4; }
    if(estadoRuta.name == 'mycolombia'){   $scope.categoriaid = 5; }

    let usu_id = $('#usu_id').val();
    let nivel_id = $('#nivel_usu_id').val();
    ...
    $scope.reloadEscribSelect = function () {
        ...
        let classEsc = 'js-escribir';
        let classSelect = 'c-inputRadio';
    ...
    $scope.guardarRecord = function(aud_titulo, audio_id){
        ...
        let audio_file = (audio_id == 1) ? $('#recorded-audio-mainAudio').attr('src') :
                         (audio_id == 2) ? $('#recorded-audio-mainAudio2').attr('src') :
                         (audio_id == 3) ? $('#recorded-audio-mainAudio3').attr('src') :
                         (audio_id == 4) ? $('#recorded-audio-mainAudio4').attr('src') :
                         (audio_id == 5) ? $('#recorded-audio-mainAudio5').attr('src') :
                         (audio_id == 6) ? $('#recorded-audio-mainAudio6').attr('src') :
                         (audio_id == 7) ? $('#recorded-audio-mainAudio7').attr('src') :
                         (audio_id == 8) ? $('#recorded-audio-mainAudio8').attr('src') :
                         (audio_id == 9) ? $('#recorded-audio-mainAudio9').attr('src') :
                         (audio_id == 10) ? $('#recorded-audio-mainAudio10').attr('src') : '';
        ...
        ApiActividades.ListarPreguntasGen(variables).then(function(data){
            ...
            if(nivel_id == 4){nivel='B2'}
            if(nivel_id == 5){nivel='C1'}
        ...
        alertify.confirm("Are you sure ?",
            ...
            function(){
                ...
                ApiActividades.ListarPreguntasGen(variables).then(function(data){
                    ...
                    let arreglodos;
                    let arreglotres;
                    let arreglo = respuesta[0].respuestas.split("|");
                    let cantPregunta = respuesta[0].cantidad_pre;
                    let preguntaid = respuesta[0].id;
                    let categoriaid = respuesta[0].categoria_id;
        ...
        alertify.confirm("Are you sure ?",
            ...
            function(){
                ...
                ApiActividades.ListarPreguntasGen(variables).then(function(data){
                    ...
                    // console.log(data.data);
                    var respuesta=data.data;
                    let arreglo1 =  ['nice',       'nice',       'nice',          'excellent', 'excellent', 'excellent',                    'horrible', 'horrible', 'horrible' ];
                    let arreglo2 =  ['lovely',     'lovely',     'lovely',         'warm', 'warm', 'warm',                                  'very loud', 'very loud', 'very loud' ];
                    let arreglo3 =  ['delicious',  'delicious',  'delicious',     'helpful', 'helpful', 'helpful',                          'average', 'average', 'average' ];
                    let arreglo4 =  ['too sweet',  'too sweet',  'too sweet',     'good', 'good', 'good',                                   'tasty', 'tasty', 'tasty' ];
                    let arreglo5 =  ['cold',       'cold',       'cold',          'interesting', 'interesting', 'interesting',              'too sour', 'too sour', 'too sour' ];
                    let arreglo6 =  ['relaxing',   'relaxing',   'relaxing',      'delicious', 'delicious', 'delicious',                    'not expensive', 'not expensive', 'not expensive' ];
                    let arreglo7 =  ['quite good', 'quite good', 'quite good',    'nice', 'nice', 'nice', 'not great',                      'not great', 'not great', 'not great' ];
                    let arreglo8 =  ['good',       'good',       'good',          'black and white', 'black and white', 'black and white',  'not great', 'not great', 'not great' ];
                    let arreglo9 =  ['enjoyable',  'enjoyable',  'enjoyable',     'famous', 'famous', 'famous',                             'not great', 'not great', 'not great' ];
                    let arreglo10 = ['enjoyable',  'enjoyable',  'enjoyable',     'not cheap', 'not cheap', 'not cheap',                    'not great', 'not great', 'not great' ];
                    let arreglo11 = ['enjoyable',  'enjoyable',  'enjoyable',     'not expensive', 'not expensive', 'not expensive',        'not great', 'not great', 'not great' ];

                    let cantPregunta = respuesta[0].cantidad_pre;
                    let preguntaid = respuesta[0].id;
                    let categoriaid = respuesta[0].categoria_id;

                    funcionesGeneral.completarTextoAct14(arreglo1,
                                                         arreglo2,
                                                         arreglo3,
                                                         arreglo4,
                                                         arreglo5,
    ...
    $scope.enviarTexto = function(){
        ...
        if(textousu == '' || textousu == null || textousu == undefined){
            ...
            alertify.confirm("Are you sure you want to send the text?",
    ...
    $scope.juegoHangman = function (aRta, nom) {
        ...
        let inicio = 1
            ...
            , iniciar = function (a) {
                ...
                for (let i = 0; i < aRta[a][0].length; i++) {

                    let casilla = $('<div>');

                    casilla
                        .addClass(`${name_game}--${name_letra}`)
                        .attr('id', `${name_letra}_${i}`);
            ...
            , pulsarTecla = function () {
                ...
                let temp = 0;
                for (let i = 0; i < aRta[inicio][0].length; i++) {
                ...
                if (temp === 0) {
                    ...
                    if (calcular === aRta[inicio][0].length) {
                        ...
                        alertify.confirm("Very well, keep it up", () => {
...
// Funcion unidad - Completar texto y selección multiple
    ...
    $scope.MostrarIframe = function(){
        ...
        ApiActividades.ListarPreguntasGen(variables).then(function(data){
            ...
            funcionesGeneral.MostrarIframe(preguntaid,usu_id).then(function(data){
                    ...
                    if(respuesta2 == '' || respuesta2 == null || respuesta2 == 'null' || respuesta2 === 'undefined'){
        ...
        alertify.confirm("Are you sure ?",
            ...
            function(){
                ...
                ApiActividades.ListarPreguntasGen(variables).then(function(data){
                    ...
                    var respuesta=data.data;
                    let preguntaid = respuesta[0].id;
                    let iframevideo= $scope.iframevideo;
    ...
    $scope.NewDoc = function () {
        ...
        if(nomDoc == ''){
            ...
            alertify.warning("<b>Alert!</b> You must upload a PowerPoint document");
               ...
               alertify.confirm("Do you want to send the document?",
    ...
    $scope.validarListadoSelect = function(){
        ...
        $(opciones).each(function (index, val) {
            ...
            if (option != null) {
                if (option == 1) { $('.opc1').css('display', 'none'); }
                if (option == 2) { $('.opc2').css('display', 'none'); }
                if (option == 3) { $('.opc3').css('display', 'none'); }
                if (option == 4) { $('.opc4').css('display', 'none'); }
                if (option == 5) { $('.opc5').css('display', 'none'); }
                if (option == 6) { $('.opc6').css('display', 'none'); }
                if (option == 7) { $('.opc7').css('display', 'none'); }
                if (option == 8) { $('.opc8').css('display', 'none'); }
            }
